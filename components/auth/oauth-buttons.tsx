...
/**
 * Google + Facebook social sign-in, shared by the login and register pages.
 * Uses Supabase OAuth, which redirects to the provider and back through
 * /auth/callback (where the code is exchanged into a session).
 */
export function OAuthButtons() {
  ...
  async function signIn(provider: "google" | "facebook") {
    ...
    // On success the browser is redirected away; only reset on error.
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2.5 rounded-lg border border-input bg-background px-4 py-2.5 text-sm font-medium shadow-sm transition-colors hover:bg-muted disabled:opacity-50"
      >
